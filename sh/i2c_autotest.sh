...
if [ "0" -ne "$?" ]; then
	insmod "${MOD_DIR}/i2c-test.ko"
fi

cat /proc/modules | grep "i2c_dev" > /dev/null
if [ "0" -ne "$?" ]; then
	insmod "${MOD_DIR}/i2c-dev.ko"
fi
...
if [ "$mode" == "tt" ] ; then
	...
	do
		...
		for i in `seq $num`
		do
			$BIN_DIR/i2c_test $msg_num $msg_len | savelog
			if [ "$?" == "0" ] ;then
		...
		echo  "Transmit speed is $speed B/s." | savelog
		let "avarge_speed=avarge_speed + speed"
	done
	echo "Transmit avarge speed is $((avarge_speed/num)) B/s." | savelog
	exit 0
elif [ "$mode" == "et" ]; then
	:
